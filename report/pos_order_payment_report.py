    ...
    def _payment_query(self):
        ...
        return """
            SELECT
                ...
                ppm.journal_id AS journal_id,
                (
                    CASE
                        WHEN po.amount_total = 0 THEN sl.amount
                        ELSE sl.amount / po.amount_total * pol.price_subtotal_incl
                    END
                ) AS amount
            FROM pos_order_line AS pol
            INNER JOIN pos_order AS po ON pol.order_id = po.id
            INNER JOIN pos_payment AS sl ON pol.order_id = sl.pos_order_id
            INNER JOIN pos_payment_method ppm ON sl.payment_method_id = ppm.id
        """
